...
presupuesto(W,X,Y):-transporte(X,A),alojamiento(Y,X,B),Z is A+B,(Z<W;Z==W).
...
signos_rango(D1,M1,D2,M2) :- (M1=<M2+1), (O is (M1+1)), signo(D1,M1,S), write(S), nl, signos_rango(D1,O,D2,M2).
